...
public class DefaultMovieService : IMovieService
{
    ...
    public async Task<Result<PagedResult<Movie>>> ReadMoviesByActorId(int actorId, int page, int size)
    {
        if (page < 1)
        {
            return new Result<PagedResult<Movie>>(
        ...
        var pagedResult = await movieRepository.ReadMoviesByActorId(actorId, page, size);
        return new Result<PagedResult<Movie>>(pagedResult, (int)HttpStatusCode.OK);
    }
